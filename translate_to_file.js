...
const timeLineRegExp = /^(\d{2}:\d{2}:\d{2}.\d{3}), (\d{2}:\d{2}:\d{2}.\d{3})/gm;
const txtRegEx =
	/^([а-яА-Яa-zA-Z\[\"\.][а-яА-Я\w \-\[\]\.,\"\']+\s([а-яА-Яa-zA-Z\[][а-яА-Я\w \-\[\]\.,\"\']+)?)/gm;

// console.log(sub.match(timeLineRegExp).length);
// console.log(sub.match(txtRegEx).length);

const timestamps = sub.match(timeLineRegExp);
...
// axios(url).then((resp) => console.log(resp.data[0][0][0]));



// axios(googleTranslateUrl("ru", "en", "дайте мне воды")).then((resp) => {
// 	let phrase = resp.data[0][0][0];
// 	console.log(phrase);
// });
